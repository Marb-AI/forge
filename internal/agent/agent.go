...
func Main(args []string) int {
	if len(args) == 0 {
		return emitError("usage: forge-agent <workspace-create|workspace-delete|workspace-list|workspace-adopt|workspace-status|workspace-activity|workspace-track|workspace-track-inc|workspace-usage|workspace-port-block|workspace-ports|workspace-container|claude-chat-send|claude-chat-tail|claude-chat-history|host-port-range|authorize-key|host-stats|version>")
	}
	switch args[0] {
	...
	case "host-port-range":
		return opPortRange(args[1:])
	case "authorize-key":
		return opAuthorizeKey(args[1:])
